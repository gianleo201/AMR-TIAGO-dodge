function [d, dot_d, ddot_d]=DistanceNDerivatives(P, r, P_obs, r_obs, x, c_obs, dx)
    assume(x, 'real')
    O=P_obs;
    vec_d1=(P-O);
    d=simplify(((vec_d1.')*(vec_d1))^(1/2)-(r_obs+r));
    dot_d = simplify(jacobian(d,[x;c_obs(1:2)])*[dx.';c_obs(3:4)]);
    ddot_d = simplify(jacobian(dot_d,[x;c_obs(1:2)])*[dx.';c_obs(3:4)]);
