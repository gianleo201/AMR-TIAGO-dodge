...
P(1,1)= {[x1 , 0]};
H(1,1)= simplify(((P{1,1}-P_obs)*(P{1,1}-P_obs).')^(1/2) - (r_obs + (1/(2*n_point_link+2))*(hb + ht + rw)));
P(2,1)= {[x1 - rt1 + rt4 , hb + ht + rw]};
H(2,1)= simplify(((P{2,1}-P_obs)*(P{2,1}-P_obs).')^(1/2) - (r_obs + (1/(2*n_point_link+2))*l2));
P(3,1)= {[x1 - rt1 + rt4 - l2*sin(x2) , hb + ht + rw + l2*cos(x2)]};
H(3,1)= simplify(((P{3,1}-P_obs)*(P{3,1}-P_obs).')^(1/2) - (r_obs + (1/(2*n_point_link+2))*l3));
P(4,1)= {[x1 - rt1 + rt4 - l3*sin(x2 + x3) - l2*sin(x2) , hb + ht + rw + l3*cos(x2 + x3) + l2*cos(x2)]};
H(4,1)= simplify(((P{4,1}-P_obs)*(P{4,1}-P_obs).')^(1/2) - (r_obs + (1/(2*n_point_link+2))*l3));

i=1;
while i <= n_point_link
    %link1
    P(1,i+1)= {[x1 - rt1 + rt4 , (i/(n_point_link+1))*(hb + ht + rw)]}; 
    H(1,i+1)= simplify(((P{1,i+1}-P_obs)*(P{1,i+1}-P_obs).')^(1/2) - (r_obs + (1/(2*n_point_link+2))*(hb + ht + rw)));
    %link2
    l2_i= (i/(n_point_link+1))*l2;
    P(2,i+1)= {[x1 - rt1 + rt4 - l2_i*sin(x2) , hb + ht + rw + l2_i*cos(x2)]};
    H(2,i+1)= simplify(((P{2,i+1}-P_obs)*(P{2,i+1}-P_obs).')^(1/2) - (r_obs + (1/(2*n_point_link+2))*l2));
    %link3
    l3_i= (i/(n_point_link+1))*l3;
    P(3,i+1)={[x1 - rt1 + rt4 - l3_i*sin(x2 + x3) - l2*sin(x2) , hb + ht + rw + l3_i*cos(x2 + x3) + l2*cos(x2)]};
    H(3,i+1)= simplify(((P{3,i+1}-P_obs)*(P{3,i+1}-P_obs).')^(1/2) - (r_obs + (1/(2*n_point_link+2))*l3));
    i = i+1;
end
DHDT2=cell([4 1]);
DHDT1=cell([4 1]);
